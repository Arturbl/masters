...
if shortest_path:
    shortest_path_coordinates = [(point[0]*m.width, point[1]*m.width) for point in shortest_path]
...
for i in range(1, len(shortest_path_coordinates)):
    direction = calculate_direction(shortest_path_coordinates[i - 1], shortest_path_coordinates[i])
    print(f"{shortest_path_coordinates[i - 1]} - {direction}")
